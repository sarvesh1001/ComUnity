...
// User model
// User model
// Update your User struct (add these fields):
type User struct {
    ID               uuid.UUID   `gorm:"type:uuid;primary_key;default:gen_random_uuid()"`
    PhoneNumber      string      `gorm:"size:15;uniqueIndex;not null"`
    Username         *string     `gorm:"size:50;uniqueIndex"`
    DisplayName      *string     `gorm:"size:100"`                    // ✅ NEW
    PreferredLanguages []string   `db:"preferred_languages"` // new (TEXT[])
    VerificationStatus string    `gorm:"size:15;default:'NONE'"`      // ✅ NEW
