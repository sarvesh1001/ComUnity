...
func (h *Helper) GenerateCertificate(ctx context.Context, subject string, san []string, validityDays int) ([]byte, []byte, error) {
	// Generate private key
	priv, err := rsa.GenerateKey(rand.Reader, 4096)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to generate private key: %w", err)
	}
	defer Wipe(priv.D.Bytes())
	...
	defer Wipe(privBytes)

	return derBytes, privBytes, nil
}

// EncryptCertificate encrypts certificate and private key with KMS
func (h *Helper) EncryptCertificate(ctx context.Context, certBytes, keyBytes []byte) ([]byte, []byte, error) {
	encCert, err := h.EncryptSmall(ctx, certBytes)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to encrypt certificate: %w", err)
	}

	encKey, err := h.EncryptSmall(ctx, keyBytes)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to encrypt private key: %w", err)
	}

	return encCert, encKey, nil
}

// DecryptCertificate decrypts certificate and private key
func (h *Helper) DecryptCertificate(ctx context.Context, encCert, encKey []byte) ([]byte, []byte, error) {
	certBytes, err := h.DecryptSmall(ctx, encCert)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to decrypt certificate: %w", err)
	}

	keyBytes, err := h.DecryptSmall(ctx, encKey)
	if err != nil {
...
// Add this method to the DataKey struct in security/kms_helper.go

// GetPlaintext returns the plaintext bytes (implements the interface expected by JWT manager)
func (dk *DataKey) GetPlaintext() []byte {
    return dk.Plaintext
}

// Add an adapter method to Helper to match the JWT interface expectations
func (h *Helper) GenerateDataKeyForJWT(ctx context.Context, keySpec string) (interface{ GetPlaintext() []byte }, error) {
    // Convert string to kmstypes.DataKeySpec
    var spec kmstypes.DataKeySpec
    switch keySpec {
    case "AES_256":
        spec = kmstypes.DataKeySpecAes256
    case "AES_128":  
        spec = kmstypes.DataKeySpecAes128
    default:
        spec = kmstypes.DataKeySpecAes256 // default
    }
